...
use Botble\CarRentals\Services\ExchangeRates\ApiLayerExchangeRateService;
use Botble\CarRentals\Services\ExchangeRates\ExchangeRateInterface;
use Botble\CarRentals\Services\ExchangeRates\OpenExchangeRatesService;
...
class CurrencySupport
{
    ...
    public function getApplicationCurrency(): ?Currency
    {
        ...
        if (! $currency->is_default) {
            $currency = $this->setCurrencyExchangeRate($currency);
        }
    ...
    public function setCurrencyExchangeRate(Currency $currency): Currency
    {
        $provider = get_car_rentals_setting('exchange_rate_api_provider', 'none');

        if ($provider !== 'none') {
            $cacheKey = 'car_rentals_currency_exchange_rate_for_' . strtoupper($currency->title);

            $rate = Cache::remember($cacheKey, 86_400, function () use ($currency) {
                try {
                    $service = $this->getExchangeRateService();

                    if (! $service) {
                        return $currency->exchange_rate ?: 1;
                    }

                    $rates = $service->cacheExchangeRates();

                    return Arr::get($rates, strtoupper($currency->title), 1);
                } catch (Throwable) {
                    return $currency->exchange_rate ?: 1;
                }
            });
    ...
    public function getExchangeRateService(): ?ExchangeRateInterface
    {
        $provider = get_car_rentals_setting('exchange_rate_api_provider', 'none');

        return match ($provider) {
            'open_exchange_rate' => app(OpenExchangeRatesService::class),
            'api_layer' => app(ApiLayerExchangeRateService::class),
            default => null,
        };
    }
}
