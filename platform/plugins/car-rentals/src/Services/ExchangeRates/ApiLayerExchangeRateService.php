...
class ApiLayerExchangeRateService implements ExchangeRateInterface
{
    ...
    {
        if (! get_car_rentals_setting('api_layer_api_key')) {
    ...
    public function cacheExchangeRates(): array
    {
        ...
        $rates = Cache::remember('car_rentals_currency_exchange_rate', 86_400, function () use ($params) {
    ...
    protected function request(array $params): array
    {
        $response = Http::baseUrl('https://api.apilayer.com')
            ->withoutVerifying()
            ->withHeaders(['apikey' => get_car_rentals_setting('api_layer_api_key')])
            ->acceptJson()
