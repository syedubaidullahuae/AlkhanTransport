...
<div {!! $shortcode->htmlAttributes() !!} class="shortcode-brands brand-style-2 py-96 background-100">
    <div class="container">
        <div class="box-search-category">
            @if(empty($shortcode->title === false))
                <h2 class="heading-3 mb-0 shortcode-title wow fadeInUp">{!! BaseHelper::clean($shortcode->title) !!}</h2>
            @endif
            <div class="d-flex align-items-center justify-content-between">
                
                @if($shortcode->subtitle)
                    
                    <p class="text-lg-medium shortcode-subtitle wow fadeInUp">{{ $shortcode->subtitle }}</p>
                @endif
                @if($shortcode->button_label)
                    <a href="{{ $shortcode->button_url }}" class="text-sm-bold neutral-1000 wow fadeInUp">
                ...
                @foreach($clients as $client)
                    <div class="brand-logo">
                        <a href="#" class="item-brand">
                            <img src="{{ RvMedia::getImageUrl($client['logo']) }}" 
